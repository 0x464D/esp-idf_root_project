...
/*
    Kconfig.projbuild: Permite configurar diferentes placas basadas en el mismo código
    Se define una sola placa en el menú de configuración (sdkconfig)
 */
#if CONFIG_BOARD_LILYGO
...
#else
static const char *TAG = "UNKNOWN_BOARD";
#endif
...
// ---- Crear UI demo ----
__attribute__((unused)) static void create_demo_ui(void)
{
    ...
    while (1)
    {
        ...
        if (ticks >= 100)
        {
            ...
            { // Se toma el mutex para evitar que otra tarea acceda a LVGL
                ...
                int required_len = label_len + contador_len + 3; // +3 por ": " y el terminador de cadena '\0'
